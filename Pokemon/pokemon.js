...
function ajax() {  
    ...
    //let url = pokemonNumber ? `https://pokeapi.co/api/v2/pokemon/${20}` : `https://pokeapi.co/api/v2/pokemon/${25}`;
...
function procesar() {
    ...
    let spriteDetras = datos.sprites.back_default;
    ...
    speciesRequest.onreadystatechange = function() {
        if (speciesRequest.readyState === 4 && speciesRequest.status === 200) {
            ...
            <img src="${spriteDetras}" alt="${nombree}">
            `;
            pokemon1.innerHTML = `
            <img src="${sprite}" alt="${nombree}">
            `;
